    ...
    match event {
        ...
        InputEvent::TouchUp { event } => {
            ...
            if forward_to_wayland {
                ...
                match shell_view {
                    crate::shell::ShellView::Home => {
                        // Check if touch was on keyboard overlay
                        if touch_on_keyboard {
                            // Handle keyboard touch for QML home screen
                            if let Some(pos) = last_pos {
                                info!("Home keyboard TouchUp at ({}, {})", pos.x, pos.y);
                                ...
                                for action in actions {
                                    ...
                                    match action {
                                            ...
                                            if let Some(c) = ch.chars().next() {
                                                ...
                                                if let Some((keycode, needs_shift)) = char_to_evdev(c) {
                                                    if let Some(keyboard) = state.seat.get_keyboard() {
                                                        ...
                                                        info!("Home: Injecting key '{}' keycode={}", c, xkb_keycode);
                                                        if needs_shift {
                                        ...
                                        KeyboardAction::Space => {
                                            state.shell.word_predictor.add_char(' ');
                                            let predictions = state.shell.word_predictor.get_predictions();
                                            if let Some(ref slint_ui) = state.shell.slint_ui {
                                                slint_ui.set_keyboard_predictions(
                                                    predictions.get(0).map(|s| s.as_str()).unwrap_or(""),
                                        ...
                                        _ => {} // Ignore other actions for now
                                    }
                                }
                            }
                        } else {
                            // Normal home touch handling (not on keyboard)
                            // Forward to Slint
                            if let Some(pos) = last_pos {
                                if let Some(ref slint_ui) = state.shell.slint_ui {
                                    slint_ui.dispatch_pointer_released(pos.x as f32, pos.y as f32);
                    ...
                    crate::shell::ShellView::App | crate::shell::ShellView::LockScreen => {
                        ...
                        if system_menu_showing {
                            if let Some(pos) = last_pos {
                                info!("System menu TouchUp at ({}, {})", pos.x, pos.y);
                                if let Some(ref slint_ui) = state.shell.slint_ui {
                                    slint_ui.dispatch_pointer_released(pos.x as f32, pos.y as f32);
                                }
                            }
...
fn try_import_egl_buffer(
    ...
    let query_fn = match display.egl_query_wayland_buffer {
        Some(f) => f,
        None => {
            tracing::trace!("try_import_egl_buffer: no egl_query_wayland_buffer");
            return None;
        }
    };
    let create_image_fn = match display.egl_create_image {
        Some(f) => f,
        None => {
            tracing::trace!("try_import_egl_buffer: no egl_create_image");
            return None;
        }
    };
    let image_target_fn = match display.gl_egl_image_target_texture_2d {
        Some(f) => f,
        None => {
            tracing::trace!("try_import_egl_buffer: no gl_egl_image_target_texture_2d");
            return None;
        }
    };
    ...
    let buffer_ptr: Option<*mut std::ffi::c_void> = with_states(wl_surface, |data| {
        ...
        if let Some(buffer_data) = data.data_map.get::<RefCell<SurfaceBufferData>>() {
            ...
            if let Some(ptr) = bd.wl_buffer_ptr {
                tracing::trace!("try_import_egl_buffer: using stored buffer ptr {:?}", ptr);
                return Some(ptr);
            }
        }
        ...
        if let Some(ref buffer_assignment) = attrs.buffer {
            ...
            match buffer_assignment {
                BufferAssignment::NewBuffer(buffer) => {
                    ...
                    tracing::trace!("try_import_egl_buffer: found NewBuffer {:?}", buffer.id());
                    Some(buffer.id().as_ptr() as *mut std::ffi::c_void)
                }
                BufferAssignment::Removed => {
                    tracing::trace!("try_import_egl_buffer: buffer was Removed");
                    None
                }
            }
        } else {
            tracing::trace!("try_import_egl_buffer: no buffer assignment");
            None
        }
    });
    ...
    let buffer_ptr = match buffer_ptr {
        Some(p) => p,
        None => {
            tracing::trace!("try_import_egl_buffer: no buffer pointer available");
            return None;
        }
    };
    ...
    tracing::trace!("EGL buffer query: {}x{}, format={}", width, height, texture_format);
    ...
    tracing::trace!("Created EGL image: {:?}", egl_image);
    ...
    tracing::trace!("Created GL texture {} from EGL image", texture_id);
        ...
        if shell_view == ShellView::App && !switcher_gesture_preview && !state.shell.lock_screen_active {
            ...
            if let Some(window) = app_window {
                ...
                if let Some(wl_surface) = wl_surface {
                    ...
                    if needs_import {
                        ...
                        if let Some(imported) = try_import_egl_buffer(&wl_surface, display) {
                            ...
                            compositor::with_states(&wl_surface, |data| {
                                ...
                                if let Some(buffer_data) = data.data_map.get::<RefCell<SurfaceBufferData>>() {
                                    ...
                                    if let Some(buffer) = bd.pending_buffer.take() {
                                        buffer.release();
                                        tracing::trace!("Released wl_buffer after EGL import");
                                    }
                                }
                            });
                            ...
                            if log_frame {
                                tracing::trace!("EGL IMPORT+RENDER[{}] frame {}: texture_id={}, {}x{}", i, frame_num, imported.0, imported.1, imported.2);
                            }
                            unsafe {
                        ...
                        let window_pos = state.space.element_location(window).unwrap_or_default();
                        if log_frame {
                            tracing::trace!("EGL RENDER[{}] frame {}: texture_id={}, {}x{}", i, frame_num, texture_id, width, height);
                        }
                        unsafe {
                            gl::render_egl_texture_at(texture_id, width, height, display.width, display.height,
                                                       window_pos.x, window_pos.y);
            ...
            if let Some(ref slint_ui) = state.shell.slint_ui {
                ...
                if keyboard_visible || volume_visible || context_menu_visible || system_menu_visible {
                    ...
                    slint_ui.set_show_volume_overlay(volume_visible);
                    slint_ui.set_volume(state.system.volume as i32);
                    slint_ui.set_muted(state.system.muted);
