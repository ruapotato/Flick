...
/// Get the command to launch the QML home screen
pub fn get_home_command() -> (String, Vec<String>) {
    let wrapper_path = get_flick_root()
        .join("apps/home/run_home.sh")
        .to_string_lossy()
        .to_string();
...
/// Path to app launch signal file (written by QML home when user taps an app)
pub fn launch_signal_path() -> PathBuf {
    get_state_dir().join("launch_app")
}

/// Get the state directory for Flick
pub fn get_state_dir() -> PathBuf {
    // Use FLICK_STATE_DIR if set
    if let Ok(state_dir) = std::env::var("FLICK_STATE_DIR") {
    ...
    let home = if let Ok(sudo_user) = std::env::var("SUDO_USER") {
        format!("/home/{}", sudo_user)
    } else if let Ok(flick_user) = std::env::var("FLICK_USER") {
        format!("/home/{}", flick_user)
    } else {
        std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string())
    };
    PathBuf::from(home).join(".local/state/flick")
}

/// Path to apps.json file (generated by compositor for QML home to read)
...
pub struct Shell {
    ...
    /// Whether QML home screen has been launched
    pub qml_home_launched: bool,
    /// Whether apps.json has been generated for QML home
    pub apps_json_generated: bool,
}
...
impl Shell {
    pub fn new(screen_size: Size<i32, Logical>) -> Self {
        ...
        let mut shell = Self {
            ...
            qml_home_launched: false,
            apps_json_generated: false,
        };
    ...
    /// Launch the external QML home screen app
    pub fn launch_home_app(&mut self, socket_name: &str) -> bool {
        if self.qml_home_launched {
            return false;  // Already launched
        }

        // Generate apps.json before launching
        if !self.apps_json_generated {
            if apps::generate_apps_json(&self.app_manager) {
                self.apps_json_generated = true;
            }
        }

        let (cmd, args) = get_home_command();
        tracing::info!("Launching QML home screen: {} {:?}", cmd, args);

        // Check if the script exists
        if !std::path::Path::new(&cmd).exists() {
            tracing::warn!("QML home script not found at: {}", cmd);
            return false;
        }
        ...
        // Clear any stale launch signal
        let launch_path = launch_signal_path();
        ...
        match std::process::Command::new("sh")
            ...
            .env("FLICK_STATE_DIR", &state_dir_str)
            .env("XDG_RUNTIME_DIR", std::env::var("XDG_RUNTIME_DIR").unwrap_or_default())
            .env("QT_SCALE_FACTOR", &qt_scale)
            .env("QT_FONT_DPI", format!("{}", (96.0 * self.text_scale) as i32))
            .env("GDK_SCALE", &gdk_scale)
        ...
        {
            Ok(_) => {
                tracing::info!("QML home screen app launched successfully");
                self.qml_home_launched = true;
                true
            }
            Err(e) => {
                tracing::error!("Failed to launch QML home screen app: {}", e);
                false
            }
        }
    }
    ...
    pub fn check_launch_signal(&self) -> Option<String> {
        let launch_path = launch_signal_path();
        if launch_path.exists() {
            match std::fs::read_to_string(&launch_path) {
                Ok(content) => {
                    ...
                    let _ = std::fs::remove_file(&launch_path);
                    let exec = content.trim().to_string();
                    if !exec.is_empty() {
                        tracing::info!("Launch signal received: {}", exec);
                        return Some(exec);
                    }
                }
                Err(e) => {
                    tracing::warn!("Failed to read launch signal: {:?}", e);
                }
            }
        }
        None
    ...
    /// Regenerate apps.json (call when app list changes)
    pub fn regenerate_apps_json(&mut self) {
        if apps::generate_apps_json(&self.app_manager) {
            self.apps_json_generated = true;
        }
    }
